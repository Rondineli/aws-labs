...
resource "aws_iam_role" "aws-opsworks-prod-ec2-role-ire" {
  ...
  path               = "/"
  assume_role_policy = file("${path.module}/policies/aws-opsworks-prod-ec2-role-ire.json")
}

resource "aws_iam_role" "aws-opsworks-service-role-ire" {
  name               = "aws-opsworks-service-role-ire"
  path               = "/"
  assume_role_policy = file("${path.module}/policies/aws-opsworks-service-role-ire.json")
}
...
resource "aws_iam_role_policy" "aws-opsworks-service-policies-ire" {
  ...
  policy = file("${path.module}/policies/aws-opsworks-service-role.json")
}